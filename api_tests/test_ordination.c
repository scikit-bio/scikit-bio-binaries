...
void test_center_mat() {
    ...
    const unsigned int n_samples = 6;
    ...
    // fp64
    {
      ...
      skbb_center_distance_matrix_fp64(n_samples, matrix, centered);
    ...
    // mixed
    {
      ...
      skbb_center_distance_matrix_fp64_to_fp32(n_samples, matrix, centered_fp32);
    ...
    // fp32
    {
      ...
      skbb_center_distance_matrix_fp32(n_samples, matrix_fp32, centered_fp32);
    ...
    // fp32 in-place
    {
      skbb_center_distance_matrix_fp32(n_samples, matrix_fp32, matrix_fp32);
    ...
    // fp64 in-place
    {
      skbb_center_distance_matrix_fp64(n_samples, matrix, matrix);
    ...
    if (failed) {
      printf("ERROR: Center distance matrix test failed\n");
    } else {
      printf("INFO: Center distance matrix test succeeded\n");
    }
}
...
void test_pcoa_fsvd() {
    ...
    const unsigned int n_samples = 9;
    ...
    if (failed) {
      printf("ERROR: Center larger distance matrix test failed\n");
    } else {
      printf("INFO: Center larger distance matrix test succeeded\n");
    }
    // Test eigens
...

    {
      ...
      skbb_fsvd_inplace_fp64(n_samples, centered, 5, -1, eigenvalues, eigenvectors);
    ...
    }
    {
      ...
      skbb_fsvd_inplace_fp32(n_samples, centered_fp32, 5, 42, eigenvalues, eigenvectors);
    ...
    if (failed) {
      printf("ERROR: FSVD test failed\n");
    } else {
      printf("INFO: FSVD test succeeded\n");
    }
...

    {
      ...
      skbb_pcoa_fsvd_fp64(n_samples, matrix, 5, 999999, eigenvalues, samples, proportion_explained); 
    ...
    // Test PCoA fp32
    {
      ...
      skbb_pcoa_fsvd_fp32(n_samples, matrix_fp32, 5, -100, eigenvalues_fp32, samples_fp32, proportion_explained_fp32);
    ...
    // Test PCoA mixed mode
    {
      ...
      skbb_pcoa_fsvd_fp64_to_fp32(n_samples, matrix, 5, -1, eigenvalues_fp32, samples_fp32, proportion_explained_fp32);
    ...
    // test in-place
    {
      ...
      skbb_pcoa_fsvd_inplace_fp64(n_samples, matrix, 5, 1, eigenvalues, samples, proportion_explained);
    ...
    // in-plave fp32
    {
      ...
      skbb_pcoa_fsvd_inplace_fp32(n_samples, matrix_fp32, 5, -9, eigenvalues, samples, proportion_explained);
    ...
    if (failed) {
      printf("ERROR: PCoA<FSVD> test failed\n");
    } else {
      printf("INFO: PCoA<FSVD> test succeeded\n");
    }
}
...
int main(int argc, char** argv) {
    ...
    printf("\n");
    if (global_failed) {
      printf("ERROR: Some ordination tests failed\n");
    } else {
      printf("INFO: All ordination tests succeeded\n");
    }
